...
namespace SharpSpecs.Framework.Specs.ScenarioSpecs
{
    ...
    [Subject(typeof(Scenario))]
    public class when_I_have_a_scenario_with_failing_then_steps : with_scenario
    {
        Establish context = I_have_a_failing_scenario;
        Because I_run_the_scenario = run_the_scenario;
        It should_report_2_passed_steps = () => scenario.PassedStepCount.ShouldEqual(3);
        It should_report_1_failed_steps = () => scenario.FailedStepCount.ShouldEqual(1);
        It should_report_1_skipped_steps = () => scenario.SkippedStepCount.ShouldEqual(0);
    }
    ...
    public class with_scenario
    {
        ...
        {
            scenario = new FeatureWithFailingThenStepsScenario().Scenarios.First();
        }
